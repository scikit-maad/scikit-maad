...
def plot_find_rois_simple():
    ...
    # the Red-faced Spinetail *Cranioleuca erythrops*. This song can be
    # detected on the recording using the function ``find_rois_cwt`` and
    # setting frequency limits ``flims=(4500, 8000)`` and temporal length of
    # signal ``tlen=2``. The segmentation results are returned as a
    # dataframe with temporal segmentation given by the function and using
    # the frequency limits defined by the user.
    ...
    # segmented in the recording by changing the detection parameters,
    # ``flims`` and ``tlen``.
    df_chirp = find_rois_cwt(s, fs, flims=(8000, 12000), tlen=0.1, th=0.001,
                             display=True, figsize=(10, 6))
    print(df_chirp)


if __name__ == '__main__':
    plot_find_rois_simple()
