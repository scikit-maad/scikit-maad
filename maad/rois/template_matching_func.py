...
#%%
def template_matching(Sxx, Sxx_template, tn, fn, ext, peak_th,
    ...
    **kwargs: keywords pair, optional
    ...
    if (peak_distance_pixel < 1):
        raise ValueError(f'`peak_distance` must be greater or equal to spectrogram resolution: {np.diff(tn)[0]}')
    ...
    if (Sxx_template.shape[1] % 2) == 1:
        pad_width = ((0, 0,), (time_width, time_width))
    else:
        pad_width = ((0, 0,), (time_width, time_width-1))
    ...
    peaks, peak_dict = find_peaks(xcorrcoef, peak_th, threshold, peak_distance_pixel,
                                  prominence, width, wlen, rel_height, plateau_size)
    peaks_time = tn[peaks]
    
    # build rois
    template_len = tn[Sxx_template.shape[1]] - tn[0]
    rois = pd.DataFrame({'peak_time': peaks_time,
