...
def filter_multires(Sxx, kernels, npyr=4, rescale=True): 
    ...
    if npyr<2: 
        ...
        im_pyr = tuple(transform.pyramid_gaussian(Sxx, downscale=2,  
                                                  max_layer=1, channel_axis=None))  # multichannel=False
    else:     
        im_pyr = tuple(transform.pyramid_gaussian(Sxx, downscale=2,  
                                                  max_layer=npyr-1, channel_axis=None)) #multichannel=False  
        ...
        for im in im_filt: 
            ...
            if ratio[0] > 1: 
                im = transform.rescale(im, scale = ratio, mode='reflect', 
                                       channel_axis=None, anti_aliasing=True)  # multichannel=False
            else: 
                pass 
            Sxx_out.append(im) 
    else: 
